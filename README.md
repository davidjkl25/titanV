...
- [x] **Recuperación de contraseña por PIN** (forgot-password → verify-pin → reset-password)
- [x] **Validación robusta de contraseña** (mín. 8 caracteres con letras, números y símbolos)
...
- [x] **Interfaz premium oscura** (sidebar con logo, badge de rol y tarjeta de sesión; videos de fondo)
...
| POST   | `/auth/forgot-password` | Solicitar PIN de recuperación al correo  |
| POST   | `/auth/verify-pin`     | Verificar el PIN de recuperación          |
| POST   | `/auth/reset-password` | Definir nueva contraseña con el PIN       |
| GET/POST | `/usuarios/`         | Listar / Crear usuarios                   |
| PUT/DELETE | `/usuarios/{id}`   | Actualizar / Eliminar usuario             |
| POST | `/usuarios/{id}/restaurar` | Reactivar un usuario eliminado          |
